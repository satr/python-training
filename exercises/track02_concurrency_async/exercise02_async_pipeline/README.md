...
`finally` block. Sketch producer, workers, and collector as separate stages;
use sentinel values or cancellation to stop workers after production, and
`await queue.join()` to account for every item. Preserve input order by
tagging values with indexes, even though tasks may finish out of order, and
validate positive worker and queue sizes.
